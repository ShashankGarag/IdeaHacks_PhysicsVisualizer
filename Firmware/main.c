...
// We will store the handles (internal IDs) generated by the ESP32 here
...
static const uint8_t accel_value[12] = {0}; // 12 bytes to hold three 4-byte floats (X, Y, Z)
...
static const esp_gatts_attr_db_t gatt_db[ACCEL_IDX_NB] =
{
    ...
    // 2. Characteristic Value (This is where the actual sensor data goes)
    [ACCEL_IDX_CHAR_VAL] =
    ...
    // 3. Client Characteristic Configuration (Required so the phone can turn on notifications)
    [ACCEL_IDX_CHAR_CFG] =
...
void sensor_task(void *pvParameters) {
    // 1. Initialize your driver (just like we did earlier)
    ...
    uint8_t notify_data[12]; // Array to hold the bytes we send over BLE

    while(1) {
        // Read the sensor
        adc_data current_g = read_adc(&accelerometer);

        // If a phone is connected, send the data!
        if (is_connected) {
            // Pack the three floats (4 bytes each) into the 12-byte array
            ...
            // Push the notification to the phone using the handle we saved earlier
            esp_ble_gatts_send_indicate(
                global_gatts_if, 
                global_conn_id, 
                accel_handle_table[ACCEL_IDX_CHAR_VAL], // Point to the value characteristic
                sizeof(notify_data), 
                notify_data, 
                false // false means "Notify" (no confirmation required from phone)
            );
        }

        // Wait 100 milliseconds before reading again (10 updates per second)
